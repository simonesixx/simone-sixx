document.addEventListener("DOMContentLoaded", () => {
  ...
  (async () => {
    ...
    // Mode preview : utilise les produits en stockage local (admin local)
    if (preview) {
      try {
        products = window.ProductStore?.loadProducts?.() || [];
      } catch {
        products = [];
      }
    }

    // Site public : source publiée (JSON)
    if (products.length === 0) {
      try {
        const published = await window.loadPublishedProducts?.();
        if (Array.isArray(published) && published.length > 0) products = published;
      } catch {
        // ignore
      }
    }

    // Fallback : liste embarquée (products.js)
